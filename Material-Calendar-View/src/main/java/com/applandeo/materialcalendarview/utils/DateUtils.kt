 ...
 */
internal fun Calendar.getMonthAndYearDate(context: Context): String {
    val year = this.get(Calendar.YEAR).toString().substring(2)
    val month = context.resources.getStringArray(R.array.material_calendar_months_array)[this.get(Calendar.MONTH)]
    return "$year.$month"
}
